...
2 **代码文件说明**
--------------------------------------------------------------
* 音乐的重命名脚本为'.code/rename.py',分割脚本为'.code/split.py',特征提取函数在脚本'.code/Extract_feature.py'中,从数据集中提取特征并保存的脚本为'.code/data_prepare.py',大部分分类器在脚本'.code/classifier.py'中,高斯混合模型在脚本'.code/GMM.py'中,流形正则化模型在脚本'.code/MR.py'中.

* 在'.code/run.py'文件中可以测试除S3VM和MR以外的算法分类精度,S3VM和MR在各自的脚本中测试.
...
--------------------------------------------------------------
### 3.1 原始数据集
* 本项目采用mp3格式的音频数据,每30s截取片段作为特征提取数据,对数据进行情感标注.项目采用的有标签音乐片段共857个,对全部数据特征提取后的特征数组和标签存储在npy格式文件中,这些文件存在'./numpy'文件夹下.其中所有音乐片段总大小为982M,保存时域数据的'data.npy'文件大小为3.16GB,单独给出[here](https://jbox.sjtu.edu.cn/l/Y0TeIM).直接分离训练集和测试集的npy格式数据在'./labeled'文件夹下.
